#!/usr/bin/env python3
"""Qwen3-TTS — Full TRT FP16 pipeline.
- Talker prefill: FP32 CUDA (has separate KV outputs, runs once)
- Talker decode: TRT FP16 via ORT TRT EP (with KV cache, no-If model)
- Code predictor: TRT FP16 via ORT TRT EP (accumulated input, no-If model)
- Vocoder: TRT FP16 via ORT TRT EP
"""
...
# Prefill: FP32 on CUDA (has separate KV outputs, runs once)
...
# Decode: no-If model with KV cache → TRT EP
print("  Talker decode (TRT EP, first run builds engine)...")
decode_s = ort.InferenceSession("/tmp/talker_dec_kv_no_if.onnx", sess_options=o, providers=trt_ep("/tmp/trt_talker"))
dc_on = [x.name for x in decode_s.get_outputs()]

# Code predictor: no-If model, accumulated input → TRT EP
print("  Code predictor (TRT EP)...")
...
# Vocoder: TRT EP
print("  Vocoder (TRT EP)...")
voc_s = ort.InferenceSession(f"{FP32}/vocoder.onnx", providers=trt_ep("/tmp/trt_voc"))
...
def synthesize(text,lang="english",output="/tmp/tts_trt.wav",mf=60,seed=None):
    ...
    # Stack prefill KV → [28, B, 8, T, 128] for TRT decode
    ...
    for fr in range(mf):
        ...
        # Code predictor (TRT, accumulated input)
        t_cp=time.perf_counter()
        fce=ce(fc)
        accum=np.concatenate([hidden,fce],axis=1) # [1,2,1024]
        subs=[]
        ...
        # Talker decode (TRT with KV cache)
        t_d=time.perf_counter()
        ts+=1
        dpos=np.array([[[ts-1]]]*3,dtype=np.int64)
        do=decode_s.run(None,{"inputs_embeds":ne,"position_ids":dpos,"past_keys":pk,"past_values":pv})
        dm=dict(zip(dc_on,do))
        logits=dm["logits"];hidden=dm["hidden_states"]
    ...
    print(f"  CP/step:     {ca:>7.1f}ms (TRT FP16 accum)")
    print(f"  Per-step:    {da+ca:>7.1f}ms  RTF={((da+ca)/80):.2f}")
    print(f"  Vocoder:     {tv:>7.0f}ms (TRT FP16)")
...
if __name__=="__main__":
    ...
    p.add_argument("--output",default="/tmp/tts_trt.wav")
